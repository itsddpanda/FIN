...
import casparser
import os
from models import User, Folio, StatementPeriod, Scheme, Valuation, Transaction
...
def convertpdf(pdf_file_path: str, password: str, userid: str):
    """
    ...
    if (userid == "2") : 
        try:
            session = SessionLocal()
            clear_database_for_identifier(session, "test@ting.com", "email")
            session.close()
            return progress_report
        except Exception as e:
            session.close()
            logger.error(f"Error in cleaning the db {e}", exc_info=False)
            return e
